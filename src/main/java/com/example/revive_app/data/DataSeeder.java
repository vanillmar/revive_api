...
public class DataSeeder implements ApplicationRunner {
    ...
    // testOne.setUsername("testOne");
    // testOne.setPassword("12345678");
    // testOne.setEmail("test_one@system.com");
    // testOne.setPerson(person);
    ...
    // permissionRepository.saveAll(List.of(createUsers, createUser, deleteUser,
    // readUsers, readUser, readMe,
    // readEmployee, readEmployees, createEmployee, createEmployees, updateEmployee,
    // updateEmployees,
    // deleteEmployee, readAddress, readAddresses, createAddress, createAddresses,
    // updateAddress,
    // updateAddresses, deleteAddress, readDepartment, readDepartments,
    // createDepartment,
    // createDepartments, updateDepartment, updateDepartments, deleteDepartment,
    // readExams));
    ...
    // adminRole.setPermissions(Set.of(createUsers, createUser, deleteUser,
    // readUsers, readUser, readMe,
    // readEmployee, readEmployees, createEmployee, createEmployees, updateEmployee,
    // updateEmployees,
    // deleteEmployee, readAddress, readAddresses, createAddress, createAddresses,
    ...
    // studentRole.setPermissions(Set.of(readUser, readMe, readAddress,
    // readDepartment, readExams));

    // Role userRole = new Role();
    // userRole.setName(Roles.USER);
    // userRole.setDescription("Regular user with view-only permissions");
    // userRole.setPermissions(Set.of(readMe, readAddress, readDepartment,
    // readExams));
    ...
    // ExamStatus inProgressStatus = new ExamStatus(null, "In Progress", "Currently
    // taking the exam");
    // ExamStatus completedStatus = new ExamStatus(null, "Completed", "Exam
    // completed");
    // ExamStatus reviewedStatus = new ExamStatus(null, "Reviewed", "Exam has been
    // reviewed");

    // Subject instrumentRating = new Subject(null, "instrument Rating",
    // "Instruments and Electronics", "—");
    // Subject airlaw = new Subject(null, "airlaw", "Air Law", "—");
    // Subject humanPerformance = new Subject(null, "human-performance", "Human
    // Performance", "—");
    // Subject aircraftTechnical = new Subject(null, "aircraft-technical", "Aircraft
    // Technical General", "—");
    // Subject flightPlanning = new Subject(null, "flight-planning", "Flight
    // Planning and Performance", "—");
    // Subject meteorology = new Subject(null, "meteorology", "Meteorology", "—");
    // Subject generalNavigation = new Subject(null, "general-navigation", "General
    // Navigation", "—");
    // Subject radioAids = new Subject(null, "radio-aids", "Radio Aids", "—");

    // List<Subject> subjects = List.of(instrumentRating, airlaw, humanPerformance,
    // aircraftTechnical, flightPlanning,
    // meteorology, generalNavigation, radioAids);

    // Exam instrumentsRatingExam = new Exam(null, instrumentRating,
    // instrumentRating.getName(), "—", 72, 9_000,
    // readyStatus);
    // Exam airlawExam = new Exam(null, airlaw, airlaw.getName(), "—", 72, 9_000,
    // readyStatus);
    // Exam humanPerformanceExam = new Exam(null, humanPerformance,
    // humanPerformance.getName(), "—", 72, 9_000,
    // readyStatus);
    // Exam aircraftTechnicalExam = new Exam(null, aircraftTechnical,
    // aircraftTechnical.getName(), "—", 72, 9_000,
    // inProgressStatus);
    // Exam flightPlanningExam = new Exam(null, flightPlanning,
    // flightPlanning.getName(), "—", 72, 9_000, readyStatus);
    // Exam meteorologyExam = new Exam(null, meteorology, meteorology.getName(),
    // "—", 72, 9_000, readyStatus);
    // Exam generalNavigationExam = new Exam(null, generalNavigation,
    // generalNavigation.getName(), "—", 72, 9_000,
    // completedStatus);
    // Exam radioAidsExam = new Exam(null, radioAids, radioAids.getName(), "—", 72,
    // 9_000, readyStatus);

    // // exams to insert
    // List<Exam> exams = List.of(instrumentsRatingExam, airlawExam,
    // humanPerformanceExam, aircraftTechnicalExam,
    // flightPlanningExam, meteorologyExam, generalNavigationExam, radioAidsExam);

    // List<Question> questions =
    // InstrumentRatingQuestions.getQuestions(instrumentRating);

    // userRepository.saveAll(List.of(adminUser, testOne));
    // studentRepository.saveAll(List.of(studentOne));
    // // addressRepository.saveAll(List.of(addressOne, addressTwo));
    // examStatusRepository.saveAll(List.of(readyStatus, inProgressStatus,
    // completedStatus, reviewedStatus));
