...
public class EmployeeService {
    ...
    public List<EmployeeResponseDTO> createEmployees(List<EmployeeRequestDTO> employees) {
        return employeeRepository.saveAll(employees.stream().map(this::toEntity).toList()).stream().map(this::toDto)
                .toList();
    }
    ...
    private EmployeeResponseDTO toDto(Employee employee) {
        return EmployeeResponseDTO.builder().id(employee.getId()).username(employee.getUsername())
                .email(employee.getEmail()).firstname(employee.getFirstname()).lastname(employee.getLastname())
                .enabled(employee.isEnabled()).roles(employee.getRoles()).build();
    }

    private Employee toEntity(EmployeeRequestDTO employeeRequest) {
        Employee employee = new Employee(employeeRequest.getUsername(),
                passwordEncoder.encode(employeeRequest.getPassword()), employeeRequest.getEmail(),
