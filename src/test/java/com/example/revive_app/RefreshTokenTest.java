...
class RefreshTokenTest {
    ...
    // assertNotEquals(token, newToken, "New access token should be different from
    // old one");
    ...
    void testRefreshTokenWithInvalidToken() throws Exception {
        ...
        mockMvc.perform(
                post("/api/auth/refresh-token").contentType(MediaType.APPLICATION_JSON).content(invalidRefreshRequest))
    ...
    void testRefreshTokenWithExpiredToken() throws Exception {
        ...
        mockMvc.perform(post("/api/auth/refresh-token").contentType(MediaType.APPLICATION_JSON).content(refreshRequest))
                .andExpect(status().isUnauthorized());
    }
}